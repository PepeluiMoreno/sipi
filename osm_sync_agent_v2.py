"""
Agente de sincronización OSM para SIPI - VERSIÓN MEJORADA
Incluye soporte para query por área ISO (España completa) y por bbox
"""
import asyncio
...
class OSMChurchSyncAgent:
    ...
    async def fetch_churches(
        ...
        Nota: Usa timeout largo (30 min) porque la query es exhaustiva
        """
    ...
    def _build_overpass_query(
        ...
        Esta query tiene 5 criterios progresivos para capturar:
        1. Lugares de culto católicos explícitos
        2. Edificios religiosos católicos por tipo
        3. Lugares de culto cristianos sin denominación
        4. Edificios religiosos cristianos sin denominación
        5. Elementos pequeños (cruces, humilladeros, grutas)
        
        Args:
            bbox: Bounding box para área específica
            use_spain_area: Si True, usa área ISO de España
        """
        if use_spain_area:
    ...
    def _create_inmueble_from_osm(self, element: dict) -> Inmueble:
        ...
        return Inmueble(
            ...
            direccion=self._build_address(tags),
            latitud=lat,
            longitud=lon,
    ...
    def _create_osm_extension(self, inmueble_id: str, element: dict) -> InmuebleOSMExt:
        """Crea extensión OSM para el inmueble"""
    ...
    def _update_inmueble_from_osm(self, inmueble: Inmueble, element: dict):
        ...
        new_address = self._build_address(tags)
    ...
    def _get_coordinates(self, element: dict) -> Tuple[Optional[float], Optional[float]]:
        ...
        # Ways y relations tienen center (gracias a 'out tags center qt')
    ...
    def _resolve_provincia(self, lat: float, lon: float) -> Optional[str]:
        """
        Resuelve provincia_id desde coordenadas
        TODO: Implementar geocoding reverso real
        """
        return None
    
    def _resolve_localidad(self, lat: float, lon: float) -> Optional[str]:
        """
        Resuelve localidad_id desde coordenadas
        TODO: Implementar geocoding reverso real
        """
        return None
    ...
    def _build_description(self, tags: dict) -> Optional[str]:
        ...
        if wikipedia := tags.get("wikipedia"):
            parts.append(f"Wikipedia: {wikipedia}")
        
        return " | ".join(parts) if parts else None
    
    def _build_address(self, tags: dict) -> Optional[str]:
        """Construye dirección desde tags OSM"""
        parts = []
...
async def main():
    ...
    try:
        ...
        print("=== Opción 1: España completa (área ISO) ===")
        stats = await agent.sync_churches(
            use_spain_area=True,
            dry_run=True  # Cambiar a False para sincronizar realmente
        )
        
        # Opción 2: Sincronizar provincia específica
        # stats = await agent.sync_churches(
        #     provincia_nombre="Madrid",
        #     dry_run=False
        # )
        
        # Opción 3: Sincronizar bbox específico
        # stats = await agent.sync_churches(
        #     bbox=(40.3, -3.8, 40.5, -3.6),  # Madrid centro
        #     dry_run=False
        # )
